...
export async function rebalanceOrgItems(orgId: string) {
  ...
  try {
    ...
    console.log(
      `[DEBUG] Attempting to fetch subscription for clean orgId: ${JSON.stringify(cleanOrgId)}`,
    );
    const subscription =
    ...
    if (subscription?.subscriptionItems) {
      ...
      if (foundFeatures.length > 0) {
        ...
        const featureNames = foundFeatures
          .map((f) => f.name || "N/A")
          .join(", "); // Use 'N/A' if name is missing
        console.log(`✅ Subscription found for ${cleanOrgId}.`);
        console.log(`   Feature IDs: [${featureIds}]`);
        console.log(`   Feature Names: [${featureNames}]`);
      } else {
        console.log(
          `✅ Subscription found for ${cleanOrgId}, but no features listed.`,
        );
      }
    } else {
      console.log(
    ...
    if (e.status !== 404) {
      ...
      console.log(
        `ℹ️ No subscription found for ${orgId}, using defaults. Error status: ${e.status}, Error message: ${e.message}`,
      );
    }
  }
  ...
  if (foundFeatures.length > 0) {
    ...
    if (
      foundFeatures.some(
        (f) => normalizeNameToSlug(f.name) === SLUG_PRO_CUSTOMERS,
      )
    ) {
      ...
      foundFeatures.some(
        (f) => normalizeNameToSlug(f.name) === SLUG_BASIC_CUSTOMERS,
      )
    ) {
      ...
      foundFeatures.some(
        (f) => normalizeNameToSlug(f.name) === SLUG_FREE_CUSTOMERS,
      )
    ) {
    ...
    if (
      foundFeatures.some(
        (f) => normalizeNameToSlug(f.name) === SLUG_PRO_PRODUCTS,
      )
    ) {
      ...
      foundFeatures.some(
        (f) => normalizeNameToSlug(f.name) === SLUG_BASIC_PRODUCTS,
      )
    ) {
      ...
      foundFeatures.some(
        (f) => normalizeNameToSlug(f.name) === SLUG_FREE_PRODUCTS,
      )
    ) {
  ...
  const finalLimitReason =
    limitReasons.length > 0 ? limitReasons.join(", ") : "default limits";

  console.log(
    `📊 Limits set to -> Customers: ${customerLimit}, Products: ${productLimit} (Reason: ${finalLimitReason})`,
  );
  ...
  // CUSTOMERS
  const customers =
    await sql`SELECT id, status FROM customers WHERE org_id = ${orgId} AND status != 'deleted' ORDER BY id ASC`;
  for (let i = 0; i < customers.length; i++) {
    const newStatus = i < customerLimit ? "active" : "disabled";
    if (customers[i].status !== newStatus) {
      console.log(
        `   [!] CUSTOMER ${customers[i].id}: ${customers[i].status} -> ${newStatus}`,
      );
  ...
  // PRODUCTS
  const products =
    await sql`SELECT id, status FROM products WHERE org_id = ${orgId} AND status != 'deleted' ORDER BY id ASC`;
  for (let i = 0; i < products.length; i++) {
    const newStatus = i < productLimit ? "active" : "disabled";
    if (products[i].status !== newStatus) {
      console.log(
        `   [!] PRODUCT ${products[i].id}: ${products[i].status} -> ${newStatus}`,
      );
